	...
	<thead>
		...
		<th>Dated</th>
			...
			<?php
			    ...
			    $sql = "SELECT job.job_id, user.user_name, section.section_name, job.problems, job.solveproc, job.status, job.remarks, job.jobcreatedate
						FROM user, section, job
						WHERE user.user_id = job.user_id
						AND job.section_id = section.section_id ORDER BY job_id DESC";
